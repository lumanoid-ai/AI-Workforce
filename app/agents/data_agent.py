...
from app.tools.analyst_tools import analyse_data
...
You have two sources, and BOTH are our data:
1. Our recruitment database — use time_to_hire, funnel_stats,
   score_distribution, open_jobs.
2. Our uploaded datasets — use analyse_data. This covers salary data,
   market data, job data, and any CSV loaded into the workspace.

CRITICAL: analyse_data returns a "narrative" field with real figures from a
real SQL query. Report those numbers. Never say we lack data when
analyse_data has returned a narrative — that IS our data.

Rules:
- Answer with numbers from these tools only. Never estimate or invent a metric.
- If the data is empty, say so plainly.
- When analyse_data returns SQL, mention that the figures came from a query.
- If the data shows WHAT happened but not WHY, say so and name what you'd need.
...
data_agent = BaseAgent(
    ...
    tools=[
        ...
        Tool("analyse_data", analyse_data,
             "Ask a question about uploaded CSV datasets. Writes and runs real SQL, "
             "returns a narrative plus the query used. Use for anything outside the "
             "recruitment database — market data, salary data, any uploaded file",
             {"question": "str"}),
    ],
    color="#3B82F6",   # blue
)
