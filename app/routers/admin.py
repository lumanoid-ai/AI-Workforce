...
# def list_workspaces():
#     """Every team, newest first. Populates the team switcher."""
#     db = db_session()
#     try:
#         rows = db.query(Workspace).order_by(Workspace.created_at.desc()).all()
#         return [{"id": w.id, "name": w.name, "context": w.context,
...
# def workspace_tasks(workspace_id: str, limit: int = 50):
#     """A team's saved history. Newest first."""
#     db = db_session()
#     try:
#         rows = (db.query(Task)
...
#                  "answer": (t.result or {}).get("answer", ""),
#                  "artifacts": (t.result or {}).get("artifacts", []),
...
#     """Every document this team has produced, for the download list."""
#     db = db_session()
#     try:
#         task_ids = [t.id for t in db.query(Task.id)
#                     .filter(Task.workspace_id == workspace_id).all()]
#         if not task_ids:
#             return []
#         rows = (db.query(Artifact)
#                   .filter(Artifact.task_id.in_(task_ids))
#                   .order_by(Artifact.created_at.desc()).all())
#         return [{"id": a.id, "agent": a.agent, "type": a.type,
#                  "title": a.title, "created_at": a.created_at} for a in rows]
#     finally:
#         db.close()
...
from app.models import (
    Application, Artifact, Candidate, EmailLog, InterviewSlot, Job, Task, Workspace
)
from app.schemas import ImportJobsIn, InterviewResultIn, JobIn, OfferIn, SlotIn, WorkspaceIn
from app.services.application_service import make_offer, mark_interviewed
from app.services.job_service import create_job, import_external_jobs
...
def new_workspace(body: WorkspaceIn, db: Session = Depends(get_db)):
    """
    Create a team. No admin key — the frontend calls this when the user
    names a new team, and shouldn't have to hold a secret to do it.
    """
    ws = Workspace(name=body.name, context=body.context)
    db.add(ws)
    db.commit()
    db.refresh(ws)
    return {"id": ws.id, "name": ws.name, "context": ws.context,
...
# ---------------------------------------------------------------------------
# TEAMS (workspaces) — read-only, no admin key needed.
#
# The frontend calls these to fill the team switcher and to reload a team's
# saved history when the user switches back to it. A workspace IS a team;
# member names are a frontend concern and never reach the backend.
# ---------------------------------------------------------------------------
...
@router.get("/workspaces/{workspace_id}/artifacts")
def workspace_artifacts(workspace_id: str, db: Session = Depends(get_db)):
    """Every document this team has produced, for the download list."""
    task_ids = [t.id for t in db.query(Task.id)
                .filter(Task.workspace_id == workspace_id).all()]
    if not task_ids:
